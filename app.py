...
class VideoProcessor:
    ...
    def ensure_models(self, model_name: str, confidence: float) -> None:
        """Load or reload detection and tracking models on demand."""
        if (
    ...
    def configure_model(self, model_name: str, confidence: float) -> None:
        """Update detector settings without reloading unless the model changed."""
        if self.detector is None:
            return
        if self._model_name != model_name:
            self.ensure_models(model_name, confidence)
        else:
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self.detector is None or self.tracker is None:
            raise RuntimeError("Models not loaded. Call ensure_models() before processing.")
...
def main() -> None:
    ...
    if is_streamlit_cloud():
        st.info("Cloud mode: use **Upload Video** (webcam is not available on Streamlit Cloud).")
    ...
    with st.sidebar:
        ...
        source_options = ["Upload Video", "Webcam"] if is_streamlit_cloud() else ["Webcam", "Upload Video"]
        source_type = st.radio("Source", source_options, horizontal=True)
        uploaded_file = None
        webcam_index = 0
    ...
    if start_btn:
        ...
        if source_type == "Upload Video":
            ...
            if is_streamlit_cloud():
                st.error("Webcam is not available on Streamlit Cloud. Please upload a video.")
                return
            source = int(webcam_index)

        with st.spinner("Loading YOLOv8 and Deep SORT models..."):
            processor.ensure_models(selected_model, confidence)
